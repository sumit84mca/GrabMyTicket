...
export class UserService {
  ...
  authUrl: string = 'http://localhost:4000/api/User/authenticate';
  // BaseURL
  localUser!: IUser;
  constructor(
    private http: HttpClient,
    private router: Router,
    private route: ActivatedRoute // Creating a property with Variable http
  ) {
  ...
  async AuthorizeUserLogin(model: IUser): Promise<Observable<IUser>> {
    return await this.http.post<IUser>(this.authUrl, model).pipe(
      map((user) => {
  ...
  async delete(id: number) {
    return await this.http.delete(`${environment.baseUrl}/User/${id}`).pipe(
      map((x) => {
